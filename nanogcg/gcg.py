...
class GCG:
    ...
    def run(
        ...
        for messages, target in messages_and_targets:
            ...
            if config.probe_sampling_config:
                ...
                draft_target_ids = self.draft_tokenizer(
                    [target], add_special_tokens=False, return_tensors="pt"
                )["input_ids"].to(model.device, torch.int64)
                ...
                (
                    ...
                    for ids in (draft_before_ids, draft_after_ids, draft_target_ids)
                ]

                prompt_data.update(
                    {
                        "draft_before_embeds": draft_before_embeds,
                        "draft_after_embeds": draft_after_embeds,
                        "draft_target_embeds": draft_target_embeds,
                ...
                if config.use_prefix_cache:
                    with torch.no_grad():
                        ...
                        prompt_data["draft_prefix_cache"] = (
                            DynamicCache.from_legacy_cache(output.past_key_values)
                        )
        ...
        for _ in tqdm(range(config.num_steps)):
            ...
            optim_ids_onehot_grad = self.compute_token_gradient_multi_prompt(optim_ids)
            ...
            with torch.no_grad():
                ...
                if self.config.probe_sampling_config is None:
                    loss = find_executable_batch_size(
                        self._compute_candidates_loss_original_multi_prompt, batch_size
                    )(sampled_ids)
                    current_loss = loss.min().item()
                    optim_ids = sampled_ids[loss.argmin()].unsqueeze(0)
                else:
                    current_loss, optim_ids = find_executable_batch_size(
                        self._compute_candidates_loss_probe_sampling_multi_prompt,
                        batch_size,
                    )(
                        sampled_ids,
                    )
    ...
    def compute_token_gradient_multi_prompt(
        self,
        optim_ids: Tensor,
    ) -> Tensor:
    ...
    def _compute_candidates_loss_original_multi_prompt(
        self,
        search_batch_size: int,
        sampled_ids: Tensor,
    ) -> Tensor:
        ...
        if sampled_ids.shape[0] == 0:
            logger.warning(
                f"Empty sampled_ids tensor passed to _compute_candidates_loss_original_multi_prompt"
            )
    ...
    def _compute_candidates_loss_probe_sampling_multi_prompt(
        self,
        search_batch_size: int,
        sampled_ids: Tensor,
    ) -> Tuple[float, Tensor]:
        """Computes the GCG loss using probe sampling across multiple prompts.
        ...
        probe_size = max(
            1, B // probe_sampling_config.sampling_factor
        )  # Ensure at least 1
        probe_idxs = torch.randperm(B)[:probe_size].to(sampled_ids.device)

        def _compute_probe_losses_multi_prompt(
            result_queue: queue.Queue, search_batch_size: int, probe_sampled_ids: Tensor
        ) -> None:
            probe_losses = self._compute_candidates_loss_original_multi_prompt(
                search_batch_size, probe_sampled_ids
            )
            result_queue.put(("probe", probe_losses))

        def _compute_draft_losses_multi_prompt(
            result_queue: queue.Queue,
            search_batch_size: int,
            draft_sampled_ids: Tensor,
        ) -> None:
            ...
            # Compute average loss across all prompts for draft model
            draft_losses = []
            for i in range(0, B, search_batch_size):
                with torch.no_grad():
                    batch_size = min(search_batch_size, B - i)
                    ...
                    for prompt_data in self.prompt_data:
                        ...
                        if prompt_data.get("draft_prefix_cache"):
                            ...
                            for layer_idx in range(
                                len(prompt_data["draft_prefix_cache"])
                            ):
                                key, value = prompt_data["draft_prefix_cache"][
                                    layer_idx
                                ]
                                draft_prefix_cache_batch.update(
                                    key.expand(batch_size, -1, -1, -1),
                            ...
                            draft_embeds = torch.cat(
                                [
                                    self.draft_embedding_layer(draft_sampled_ids_batch),
                                    prompt_data["draft_after_embeds"].repeat(
                                        batch_size, 1, 1
                                    ),
                                    prompt_data["draft_target_embeds"].repeat(
                                        batch_size, 1, 1
                                    ),
                                ],
                                dim=1,
                            ...
                            draft_embeds = torch.cat(
                                [
                                    prompt_data["draft_before_embeds"].repeat(
                                        batch_size, 1, 1
                                    ),
                                    self.draft_embedding_layer(draft_sampled_ids_batch),
                                    prompt_data["draft_after_embeds"].repeat(
                        ...
                        draft_logits = draft_output.logits
                        tmp = (
                            draft_embeds.shape[1]
                            - prompt_data["draft_target_ids"].shape[1]
                        )
                        shift_logits = draft_logits[..., tmp - 1 : -1, :].contiguous()
                        shift_labels = prompt_data["draft_target_ids"].repeat(
                            batch_size, 1
                        )
        ...
        probe_sampled_ids = sampled_ids[probe_idxs]

        # Step 1. Compute loss of all candidates using the draft model
        draft_thread = threading.Thread(
            target=_compute_draft_losses_multi_prompt,
            args=(result_queue, search_batch_size, draft_sampled_ids),
        )

        # Step 2. In parallel to 1., compute loss of the probe set on target model
        probe_thread = threading.Thread(
            target=_compute_probe_losses_multi_prompt,
            args=(result_queue, search_batch_size, probe_sampled_ids),
        )
        ...
        filtered_sampled_ids = sampled_ids[top_indices]
        filtered_losses = self._compute_candidates_loss_original_multi_prompt(
            search_batch_size, filtered_sampled_ids
        )
        ...
        return (
            (best_probe_loss, probe_sampled_ids[probe_losses.argmin()].unsqueeze(0))
            if best_probe_loss < best_filtered_loss
            else (
                best_filtered_loss,
                filtered_sampled_ids[filtered_losses.argmin()].unsqueeze(0),
            )
        )
